...
class Align(SampleFactory, object):
	...
	def align_executable(self):
		...
		for sample in self.runsheet_data:
			...
			sam2bed_string = """| /home/sfurla/Scripts/sam2bed.py - -o %s %s""" % (sample['bed_out']+'tmp', self.filter_string)
			if self.cluster=="SLURM":
				modules = """\nsource /app/Lmod/lmod/lmod/init/bash\nml SEACR\nml bowtie2\nmodule load samtools\n"""
			else:
				modules = """\nmodule load bowtie2\nmodule load samtools\necho '\nRunning Bowtie piped to sam2bed.py...\n[BOWTIE] Output:\n'\n"""
...
class Norm(SampleFactory, object):
	...
	def norm_executable(self):
		...
		for sample in self.runsheet_data:
			...
			commandline = """genomeCoverageBed -bg -i %s -g %s -scale %s -trackline | /home/sfurla/Scripts/pyWriter.py - %s\n""" % (sample['bed_out'], sample['genome_sizes'], sample['scale_factor'], sample['bedgraph'])
			commandline = modules + commandline
			command.append(commandline)
		return command
