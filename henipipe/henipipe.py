...
class FC(SampleFactory, object):
    ...
    def FC_executable(self):
        ...
        for item in self.runsheet_data:
            ...
            commandline = commandline + """echo '\n[FC] Running SEACR on merged data... :\n'\nbash %s %s %s %s %s %s\n""" % (SEACR_SCRIPT, treat_p = os.path.join(self.out, item["FC_DIFF_treatment"]+"_combined.bedgraph"), treat_p = os.path.join(self.out, item["FC_DIFF_control"]+"_combined.bedgraph"), self.norm, self.method, item["FC_DIFF_treatment"]+"_FC_SEACR")
